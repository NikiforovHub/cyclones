...
clust_data_test = clust_data_all[,c(2,5),with = FALSE]
cl_test = kmeans(clust_data_test, centers = 5)
plot(clust_data_test, col = cl_test$cluster)
...
clust_test_cache_folder = "cache/clusterization/clust_test/"
...
clust_test_IDs = list()
for (i in 1:4){
  clust_test_IDs[[i]] = clust_data_all$ID[(cl_test$cluster == i)] 
}
...
clust_test_sum_list = list()


for (i in 1:length(clust_test_IDs)){
  clust_test_sum_list[[i]] = matrix(0, nrow = nlon, ncol = nlat)
}



for(filename in files_real_data){
  cache_name = paste0("until ",filename,".cache")
  cache_path = paste0(clust_test_cache_folder, cache_name)
  if (!file.exists(cache_path)){
    print(paste0("Start work with ", filename))
    data_filename = paste0(data_folder,filename)
    data = read_nc_file(data_filename)
    for (i in 1:length(clust_test_IDs)){
      dates = convert_to_dates(cyclones_base = cyclones_base_all, IDs = clust_test_IDs[[i]])
      ...
      clust_test_sum_list[[i]] = clust_test_sum_list[[i]] + summary
    }
    save(clust_test_sum_list, file = cache_path)
...
for (i in 1:length(clust_test_sum_list)){
  image_name = paste0("clust_test_",i,".png")
  image_path = paste0(images_folder, image_name)
  if(!file.exists(image_path)){
    data_tmp = list(lat = data$lat, lon = data$lon, sum = clust_test_sum_list[[i]])
